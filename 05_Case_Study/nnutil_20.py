{
  "nbformat": 4,
  "nbformat_minor": 0,
  "metadata": {
    "colab": {
      "name": "nnutil_20.py",
      "provenance": [],
      "authorship_tag": "ABX9TyOpjRxnWlkKWECRk1lAmqSS",
      "include_colab_link": true
    },
    "kernelspec": {
      "name": "python3",
      "display_name": "Python 3"
    }
  },
  "cells": [
    {
      "cell_type": "markdown",
      "metadata": {
        "id": "view-in-github",
        "colab_type": "text"
      },
      "source": [
        "<a href=\"https://colab.research.google.com/github/ealsk/Undergrad-DeepLearning-20Fall/blob/master/nnutil_20_py.ipynb\" target=\"_parent\"><img src=\"https://colab.research.google.com/assets/colab-badge.svg\" alt=\"Open In Colab\"/></a>"
      ]
    },
    {
      "cell_type": "code",
      "metadata": {
        "id": "QbQ3McgqK5p7"
      },
      "source": [
        "import numpy as np"
      ],
      "execution_count": 1,
      "outputs": []
    },
    {
      "cell_type": "code",
      "metadata": {
        "id": "e66mUWoeM4gp"
      },
      "source": [
        "from skimage import io\r\n",
        "import matplotlib.pyplot as plt"
      ],
      "execution_count": 2,
      "outputs": []
    },
    {
      "cell_type": "code",
      "metadata": {
        "id": "kX17Vr8FM5Yz"
      },
      "source": [
        "def imshow(image_RGB, title=''):\r\n",
        "    io.imshow(image_RGB)\r\n",
        "    \r\n",
        "    plt.title(title)\r\n",
        "    io.show()"
      ],
      "execution_count": 4,
      "outputs": []
    },
    {
      "cell_type": "code",
      "metadata": {
        "id": "8BAZX_PpM6-N"
      },
      "source": [
        "def random_crop(img, crop_size):\r\n",
        "    # Note: image_data_format is 'channel_last'\r\n",
        "    assert img.shape[2] == 3\r\n",
        "    height, width = img.shape[0], img.shape[1]\r\n",
        "    dy, dx = crop_size\r\n",
        "    x = np.random.randint(0, width - dx + 1)\r\n",
        "    y = np.random.randint(0, height - dy + 1)\r\n",
        "    return img[y:(y+dy), x:(x+dx), :]"
      ],
      "execution_count": 5,
      "outputs": []
    },
    {
      "cell_type": "code",
      "metadata": {
        "id": "-1dvk3__M9qn"
      },
      "source": [
        "def center_crop(img, crop_size):\r\n",
        "    #width, height = img.size\r\n",
        "    assert img.shape[2] == 3\r\n",
        "    height, width = img.shape[0], img.shape[1]\r\n",
        "    \r\n",
        "    new_width, new_height = crop_size [0], crop_size[1]\r\n",
        "    \r\n",
        "    x = int((width - new_width)/2)\r\n",
        "    y = int((height - new_height)/2)\r\n",
        "    dx = new_width\r\n",
        "    dy = new_height\r\n",
        "    #right = int((width + new_width)/2)\r\n",
        "    #bottom = int((height + new_height)/2)\r\n",
        "\r\n",
        "    return img[y:(y+dy), x:(x+dx), :]\r\n",
        "    "
      ],
      "execution_count": 11,
      "outputs": []
    },
    {
      "cell_type": "code",
      "metadata": {
        "id": "r7NeLqsXNCMM"
      },
      "source": [
        "def crop_generator(batches, crop_length):\r\n",
        "    \"\"\"Take as input a Keras ImageGen (Iterator) and generate random\r\n",
        "    crops from the image batches generated by the original iterator.\r\n",
        "    https://jkjung-avt.github.io/keras-image-cropping/\r\n",
        "    \"\"\"\r\n",
        "    while True:\r\n",
        "        batch_x, batch_y = next(batches)\r\n",
        "        batch_crops = np.zeros((batch_x.shape[0], crop_length, crop_length, 3))\r\n",
        "        for i in range(batch_x.shape[0]):\r\n",
        "            #batch_crops[i] = random_crop(batch_x[i], (crop_length, crop_length))\r\n",
        "            batch_crops[i] = center_crop(batch_x[i], (crop_length, crop_length))\r\n",
        "        yield (batch_crops, batch_y)"
      ],
      "execution_count": 12,
      "outputs": []
    },
    {
      "cell_type": "code",
      "metadata": {
        "id": "AebtbIfhOiym"
      },
      "source": [
        "import datetime\r\n",
        "def GetTimeString(m = -1):\r\n",
        "    if m==0:\r\n",
        "        s1 = datetime.datetime.now().strftime(\"%Y%m%d%H%M%S\")\r\n",
        "    else:\r\n",
        "        s1 = datetime.datetime.now().strftime(\"%Y%m%d_%H%M%S\")\r\n",
        "\r\n",
        "    return s1"
      ],
      "execution_count": 13,
      "outputs": []
    }
  ]
}
